...
def run_exit_engine():
    ...
    for t in open_trades:
        ...
        if entry_price > 0:
            if side == "SHORT":
                move = (entry_price - live_price) / entry_price
                if move >= PROFIT_TAKE_PCT:
                    profit_take = True
            elif side == "LONG":
                move = (live_price - entry_price) / entry_price
                if move >= PROFIT_TAKE_PCT:
                    profit_take = True

        if trade_id not in decay_ids and not bucket_rebalanced and not profit_take:
            continue
